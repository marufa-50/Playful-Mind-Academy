/* Playful Mind Academy • Screen Time Limit
...
(function(){
  ...
  const DEFAULT_ACTIVITIES = [
    ...
    { id:'learning', name:'Learning Bits', emoji:'📚', color:'#ffd95a' }, // playful micro-learning
  ...
  function drawLineChart(){
    ...
    const w = els.lineChart.width, h = els.lineChart.height;
    ctx.clearRect(0,0,w,h);

    // Create 24 buckets (minutes per hour)
    const data = getChildData(activeChildId);
    const day = todayKey();
    const buckets = new Array(24).fill(0);
    data.activities.forEach(a=>{
      (a.sessions[day]||[]).forEach(s=>{
        const hr = new Date(s.timestamp).getHours();
        buckets[hr] += s.minutes;
      });
    });

    const maxV = Math.max(30, ...buckets);
    // axes
    ctx.strokeStyle = '#ddd';
    ...
    // line
    ctx.beginPath();
    const plotX = i => 40 + (i/23)*(w-60);
    const plotY = v => (h-30) - (v/maxV)*(h-60);
    buckets.forEach((v,i)=>{
      const x = plotX(i), y = plotY(v);
      if(i===0) ctx.moveTo(x,y); else ctx.lineTo(x,y);
    });
    ctx.strokeStyle = '#4c9aef'; ctx.lineWidth = 2; ctx.stroke();

    // fill area
    ctx.lineTo(w-10,h-30); ctx.lineTo(40,h-30); ctx.closePath();
    ctx.fillStyle = 'rgba(76,154,239,0.12)'; ctx.fill();

    // hour labels
    ctx.fillStyle = '#666'; ctx.font = '10px sans-serif';
    for(let i=0;i<24;i+=3){
      const x = plotX(i);
      ctx.fillText(String(i).padStart(2,'0'), x-6, h-16);
    }
    ctx.fillText('Minutes per hour (today)', 44, 14);
  }
  ...
  window.history.back();
    ...
    els.btnSaveRules.onclick = ()=>{
      ...
      updateQuotaUI(); drawBarChart(); drawDonut(els.donutChart); drawLineChart();
      toast('Saved');
    };
    ...
    // Charts & logs
    els.btnRefreshCharts.onclick = ()=>{ drawBarChart(); drawDonut(els.donutChart); drawLineChart(); updateQuotaUI(); };
    ...
    els.btnSeedDemo.onclick = ()=> { seedDemoData(); toast('Sample activities added'); };
  ...
  function recordSession(activityId, minutes, timestamp=Date.now()){
    const data = getChildData(activeChildId);
    const day = todayKey();
    const activity = data.activities.find(a=>a.id===activityId);
    if(!activity.sessions[day]) activity.sessions[day] = [];
    activity.sessions[day].push({ minutes, timestamp });
    setChildData(activeChildId, data);
  }
  ...
  function addLog(action, activityId, minutes, displayTime){
    const data = getChildData(activeChildId);
    const day = todayKey();
    if(!data.log[day]) data.log[day] = [];
    data.log[day].push({
      time: displayTime || timeStr(new Date()),
  ...
  // Demo seeding for "Today’s Action Log" buttons
  function seedDemoData(){
    const base = new Date();
    const setTime = (h,m)=> new Date(base.getFullYear(), base.getMonth(), base.getDate(), h, m).getTime();
    // clear today first
    const data = getChildData(activeChildId);
    const day = todayKey();
    data.activities.forEach(a=> a.sessions[day] = []);
    ...
    // add sample sessions across the day
    const samples = [
      { id:'games', minutes: 25, t:setTime(9,15) },
      { id:'drawing', minutes: 20, t:setTime(11,30) },
      { id:'puzzles', minutes: 15, t:setTime(13,10) },
      { id:'help_mom', minutes: 10, t:setTime(15,00) },
      { id:'music', minutes: 12, t:setTime(17,20) },
      { id:'donation', minutes: 8, t:setTime(18,05) },
      { id:'social', minutes: 18, t:setTime(19,00) },
      { id:'games', minutes: 30, t:setTime(20,10) },
    ];
    samples.forEach(s=>{
      recordSession(s.id, s.minutes, s.t);
      addLog('Complete', s.id, s.minutes, timeStr(new Date(s.t)));
      awardStreak(s.minutes);
    });
    refreshAll();
  }
  ...
  function refreshAll(){
    ...
    renderLog();
    updateQuotaUI();
    drawBarChart(); drawDonut(els.donutChart); drawLineChart();
  }
